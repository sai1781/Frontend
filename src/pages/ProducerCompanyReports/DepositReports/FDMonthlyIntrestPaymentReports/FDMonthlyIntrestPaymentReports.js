...
import { saveAs as saveAsDocx } from 'file-saver'; // Import saveAs with a different name to avoid conflicts
      ...
      <Space size={20} className="DReports" direction="vertical">
        <div className="DReportscards">
          <Space size={10}>
            ...
            <DatePicker onChange={date => setSelectedFromDate(date)} style={{ width: '150px' }} />
            <Typography.Text>To Date:</Typography.Text>
            <DatePicker onChange={date => setSelectedToDate(date)} style={{ width: '150px' }} />
            <Typography.Text>FD-ID:</Typography.Text>
            <Dropdown overlay={generateDropdownMenu(["select", "all", ])} placement="bottomRight">
              <Button className="dropdown-select" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} >
                <span>{accountNo || "Fd_iD"}</span>
                <DownOutlined />
              </Button>
            </Dropdown>
            <Button
